...
from PySide6.QtCore import QObject, Signal, Qt, QRectF, QPropertyAnimation, QEasingCurve, QByteArray, QSize, QTimer
from PySide6.QtGui import QPixmap, QPainter, QPen, QFont, QTextOption, QTransform, QWheelEvent, QResizeEvent
from PySide6.QtOpenGLWidgets import QOpenGLWidget
from PySide6.QtWidgets import QGraphicsPixmapItem, QGraphicsView, QWidget, QStyleOptionGraphicsItem, QScrollBar, \
    QGraphicsItem, QSizePolicy, QGraphicsScene, QGraphicsProxyWidget
...
class VerticalManagement(BaseManagement):
    ...
    def adjustSceneBounds(self, items, target):
        ...
        if items:
            ...
            height = sum(item.get_height() for item in items if isinstance(item, QScalingGraphicPixmapItem)) + self.buttons_widget_proxy.widget().height()
        else:
