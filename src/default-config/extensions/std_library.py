...
class StdSaver(LibrarySaver):
    ...
    def save_chapter(cls, provider: CoreProvider, chapter_number: str, chapter_title: str, chapter_img_folder: str,
        ...
        ret_val = super().save_chapter(provider, chapter_number, chapter_title, chapter_img_folder, quality_present)
        if not ret_val:
            return False
...
class StdLibraryProvider(LibraryProvider):
    saver: _ty.Type[CoreSaver] | None = StdSaver
    ...
    def __init__(self, title: str, chapter: int, library_path: str, cache_folder: str, logo_folder: str) -> None:
        ...
        super().__init__(title, chapter, library_path, cache_folder, logo_folder, logo, None)
        self.clipping_space = (0, 0, -1, -2)

    def load_current_chapter(self, progress_queue=None) -> bool:
        ...
        for idx, src in enumerate(image_files, start=1):
            dst = os.path.join(self._cache_folder, os.path.basename(src))
            try:
